.r95_1rnz0nz0 {
  width: var(--r95-space-12);
  height: var(--r95-space-12);
  display: inline-block;
  position: absolute;
  left: var(--r95-space-0);
  top: var(--r95-space-0);
  background-image: url('data:image/png;base64,');
  image-rendering: pixelated;
}
.r95_1rnz0nz1 {
  padding: var(--r95-space-1);
  user-select: none;
  position: absolute;
  top: 0;
  left: var(--r95-space-18);
}
.r95_1rnz0nz2 {
  position: relative;
  margin-bottom: var(--r95-space-10);
  display: block;
}
.r95_1rnz0nz2:has(:disabled) {
  color: var(--r95-color-materialTextDisabled);
  text-shadow: 0.5px 0.5px var(--r95-color-materialTextDisabledShadow);
}
.r95_1rnz0nz3 {
  margin: 0;
  opacity: 0;
}
.r95_1rnz0nz3:focus ~ .r95_1rnz0nz1, &:active + .r95_1rnz0nz1 {
  border-width: var(--r95-space-1);
  border-style: dotted;
  padding: 0;
}
.r95_1rnz0nz3:checked + .r95_1rnz0nz0 {
  background-image: url('data:image/png;base64,');
}
.r95_1rnz0nz3:disabled + .r95_1rnz0nz0 {
  background-image: url('data:image/png;base64,');
}
.r95_1rnz0nz3:checked:disabled + .r95_1rnz0nz0 {
  background-image: url('data:image/png;base64,');
}