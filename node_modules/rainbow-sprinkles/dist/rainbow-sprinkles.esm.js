import { _ as _unsupportedIterableToArray, a as _defineProperty, m as mapValues, b as _toConsumableArray, c as createRuntimeFn } from './createRuntimeFn-891c4670.esm.js';
...
function createStyles(property, scale, conditions, defaultCondition) {
  ...
  if (!conditions) {
    ...
    var styleValue = _defineProperty({}, property, cssVar);
    var className = style(options['@layer'] ? _defineProperty({}, '@layer', _defineProperty({}, options['@layer'], styleValue)) : styleValue, property);
    return {
      vars: {
        "default": cssVar
      },
  ...
  var classes = mapValues(conditions, function (conditionValue, conditionName) {
    ...
    return style(options['@layer'] ? _defineProperty({}, '@layer', _defineProperty({}, options['@layer'], styleValue)) : styleValue, "".concat(property, "-").concat(conditionName));
  });
  return {
    dynamic: {
...
function createStaticStyles(property, scale, conditions, defaultCondition) {
  ...
  var values = mapValues(scaleObj, function (scaleValue, scaleKey) {
    ...
    if (!conditions) {
      return {
        "default": style(options['@layer'] ? _defineProperty({}, '@layer', _defineProperty({}, options['@layer'], styleValue)) : styleValue, "".concat(property, "-").concat(scaleKey))
      };
    }
    var classes = mapValues(conditions, function (conditionValue, conditionName) {
      var styleValue = _defineProperty({}, property, scaleValue);
      ...
      return style(options['@layer'] ? _defineProperty({}, '@layer', _defineProperty({}, options['@layer'], styleValue)) : styleValue, "".concat(property, "-").concat(scaleKey, "-").concat(conditionName));
    });
    return {
      conditions: classes,
      "default": classes[defaultCondition]
